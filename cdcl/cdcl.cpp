...
#define ACTIVITY_SCORE 10   //VSIDS得分
#define ACTIVITY_FLOAT 0.5   //(0<a<1)
#define RESTART_BASE_INTERVAL 700 // can even be 1
...
/// </summary>
/// <param name="v"></param>
void bump_activity_vsids(uint v)
{
    activity[v] += ACTIVITY_SCORE;
    if (activity[v] > ACTIVITY_RESCALE_LIMIT) { // rescore
        for (uint v = 1; v <= N; ++v)
            activity[v] *= ACTIVITY_FLOAT;
    }
    if (heap_index[v] != 0)
